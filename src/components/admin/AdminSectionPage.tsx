...
export default function AdminSectionPage({ section }: { section: string }) {
 ...
 return <><AdminPageHeader title={sectionTitles[section]} description={["insights","sources","price-model"].includes(section) ? "타 플랫폼의 수집 표본과 분석 결과를 확인하세요." : "운영 데이터를 확인하고 필요한 항목을 점검하세요."} action={<button onClick={retry} disabled={loading}>새로고침</button>} />
