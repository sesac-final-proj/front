import { authorizedFetch, AuthRequiredError } from "@/services/tradeService";
...
export async function getMe(): Promise<Me> {
  const response = await authorizedFetch("/api/v1/auth/me", { headers: { Accept: "application/json" } });
...
export async function updateProfile(input: { phoneNumber?: string; profileImageUrl?: string | null }): Promise<Me> {
  const response = await authorizedFetch("/api/v1/auth/me/profile", {
    method: "PUT",
...
export async function updateRegion(regionId: number, radiusM = 1000): Promise<Me> {
  const response = await authorizedFetch("/api/v1/auth/me/region", {
    method: "PUT",
    headers: { Accept: "application/json", "Content-Type": "application/json" },
...
export async function selectNickname(nickname: string): Promise<void> {
  const response = await authorizedFetch("/api/v1/nicknames/selection", {
    method: "POST",
    headers: { Accept: "application/json", "Content-Type": "application/json" },
