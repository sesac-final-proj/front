...
export default function GajiMarketApp() {
  ...
  const showBottomNav =
    (!subPage || ["my-menu", "dream-dashboard", "dream-notice", "settings", "sales", "favorites", "search", "all-services"].includes(subPage.type)) &&
    !(activeTab === "map" && mapSheetState !== "collapsed");
...
function MapScreen({
  ...
  const endDrag = useCallback(() => {
    ...
    if (velocity < -0.35 || deltaY < -55) {

      if (origin === "collapsed") {

        targetState = (velocity < -0.85 || deltaY < -150) ? "expanded" : "half";
    ...
    } else if (velocity > 0.35 || deltaY > 55) {

      if (origin === "expanded") {

        targetState = (velocity > 0.85 || deltaY > 150) ? "collapsed" : "half";
    ...
    sheetEl.style.transition = "transform 360ms cubic-bezier(0.32, 0.72, 0, 1), height 360ms cubic-bezier(0.32, 0.72, 0, 1)";
  ...
  const onGlobalPointerMove = useCallback((e: PointerEvent) => {
    ...
    if (dt > 8) {
      ...
      dragVelocityRef.current = (e.clientY - lastYRef.current) / dt;
    ...
    if (originStateRef.current === "expanded" && deltaY < 0) {

      effectiveDelta = deltaY * 0.22;

    } else if (originStateRef.current === "collapsed" && deltaY > 0) {
    ...
    currentDeltaYRef.current = effectiveDelta;
    ...
    if (sheetRef.current) {
      ...
      sheetRef.current.style.transform = `translate3d(0, ${effectiveDelta}px, 0)`;
  ...
  const handleHandleClick = (e: React.MouseEvent) => {
    ...
    const next = sheetState === "collapsed" ? "half" : sheetState === "half" ? "expanded" : "half";

    onSheetStateChange(next);

    if (sheetRef.current) {

      sheetRef.current.style.transition = "transform 360ms cubic-bezier(0.32, 0.72, 0, 1), height 360ms cubic-bezier(0.32, 0.72, 0, 1)";
  ...
  const handleSheetPointerDown = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (sheetState === "expanded" && sheetRef.current && sheetRef.current.scrollTop > 5) {
  ...
  const handleWheel = (e: React.WheelEvent<HTMLDivElement>) => {
    ...
    if (e.deltaY > 16 && sheetState !== "expanded") {
      ...
      wheelTimeoutRef.current = window.setTimeout(() => { wheelTimeoutRef.current = null; }, 360);
    ...
    <section className={styles.mapScreen}>
      ...
      >
        ...
        <button
          ...
          aria-label={sheetState === "expanded" ? "지도 목록 접기" : "지도 목록 펼치기"}
